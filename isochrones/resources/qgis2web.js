...
var expandedAttribution = new ol.control.Attribution({ collapsible: false });

var map = new ol.Map({
	controls: ol.control.defaults({attribution:false}).extend([ expandedAttribution,new measureControl() ]),
	target: document.getElementById('map'),
	renderer: 'canvas',
	overlays: [overlayPopup],
	layers: layersList,
	view: new ol.View({ maxZoom: 25, minZoom: 3 }) //   projection:'EPSG:2154', zoom:6, center:[553447,6255582]
});

var layerSwitcher = new ol.control.LayerSwitcher({tipLabel: "Layers"});
map.addControl(layerSwitcher);
var viewProjection = map.getView().getProjection();
//console.log( viewProjection.getCode() )
//var minim= ol.proj.transform([493217,6202905], lambert93, viewProjection)
//console.log( minim )
//map.getView().fit([minim[] ,6202905, 620835,6307862], map.getSize());
//map.getView().fit([0,5202905, 320835,6307862], map.getSize());
...
map.on('pointermove', function(evt) {
		...
		if( measuring ){
				/** @type {string} */
				var helpMsg = 'Cliquer pour démarrer la mesure de distance';
				if (sketch) {
...
map.on('pointermove', function(evt) { onPointerMove(evt); });
...
var continueLineMsg= 'Cliquer pour fixer le segment actuel et en démarrer un nouveau<br>Double-cliquer pour terminer la mesure';
...
function addInteraction() {
  ...
  draw.on('drawstart', function(evt) {
		this.source_.clear() //	draw.source_.clear()
